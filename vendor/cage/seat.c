...
static void
report_human_activity(struct cg_seat *seat)
{
	if (seat->activity_fd < 0 || seat->activity_socket_path[0] == '\0') {
		return;
	}
	uint64_t now = monotonic_msec();
	if (now - seat->last_activity_msec < 32U) {
		return;
	}
	seat->last_activity_msec = now;
	struct sockaddr_un address = {.sun_family = AF_UNIX};
	snprintf(address.sun_path, sizeof(address.sun_path), "%s", seat->activity_socket_path);
	char event = 'H';
	if (sendto(seat->activity_fd, &event, 1, MSG_DONTWAIT | MSG_NOSIGNAL,
		   (struct sockaddr *)&address, sizeof(address)) < 0 &&
	    errno != ENOENT && errno != ECONNREFUSED && errno != EAGAIN) {
		wlr_log(WLR_DEBUG, "AgentSeat human activity signal failed: %s", strerror(errno));
	}
}

static bool
pointer_is_virtual(struct cg_seat *seat, struct wlr_pointer *wlr_pointer)
{
...
agentseat_host_scale(void)
{
	...
	return end && end[0] == '\0' && scale >= 1.0f && scale <= 4.0f ? scale : 1.0f;
}
...
static float
host_pointer_coordinate_scale(struct cg_seat *seat, struct wlr_pointer *wlr_pointer)
{
	if (pointer_is_virtual(seat, wlr_pointer)) {
		return 1.0f;
	}
	if (agentseat_physical_layout()) {
		return agentseat_host_scale();
	}
	struct cg_output *output;
	wl_list_for_each (output, &seat->server->outputs, link) {
		const char *output_name = wlr_pointer->output_name;
		if (!output_name || strcmp(output_name, output->wlr_output->name) == 0) {
...
static void
handle_new_pointer(struct cg_seat *seat, struct wlr_pointer *wlr_pointer, bool is_virtual)
{
...
handle_virtual_pointer(struct wl_listener *listener, void *data)
{
	...
	handle_new_pointer(seat, wlr_pointer, true);
	update_capabilities(seat);
}
...
handle_keyboard_group_key(struct wl_listener *listener, void *data)
{
	...
	if (!cg_group->is_virtual) {
		report_human_activity(cg_group->seat);
	}
...
handle_keyboard_group_modifiers(struct wl_listener *listener, void *data)
{
	...
	if (!group->is_virtual) {
		report_human_activity(group->seat);
	}
...
handle_new_input(struct wl_listener *listener, void *data)
{
	...
	case WLR_INPUT_DEVICE_POINTER:
		handle_new_pointer(seat, wlr_pointer_from_input_device(device), false);
		break;
...
handle_cursor_axis(struct wl_listener *listener, void *data)
{
	...
	if (!pointer_is_virtual(seat, event->pointer)) {
		report_human_activity(seat);
	}
...
handle_cursor_button(struct wl_listener *listener, void *data)
{
	...
	if (event->state == WL_POINTER_BUTTON_STATE_PRESSED) {
		seat->pointer_press_x = seat->cursor->x;
		seat->pointer_press_y = seat->cursor->y;
	}

	press_cursor_button(seat, &event->pointer->base, event->time_msec, event->button, event->state, seat->cursor->x,
			    seat->cursor->y);
	/* Focus the surface under a press before delivering the button. This keeps
	 * the press grab and serial on the same client, which XDG interactive move
	 * requires for the embedded AgentSeat head. */
	wlr_seat_pointer_notify_button(seat->seat, event->time_msec, event->button, event->state);
	if (event->state == WL_POINTER_BUTTON_STATE_RELEASED)
		seat->moving_view = NULL;
	wlr_idle_notifier_v1_notify_activity(seat->server->idle, seat->seat);
}

static void
update_interactive_move(struct cg_seat *seat)
{
	struct cg_view *view = seat->moving_view;
	...
	if (height <= 0)
		height = 52;
	int x = (int)(seat->cursor->x - seat->move_offset_x);
	int y = (int)(seat->cursor->y - seat->move_offset_y);
	...
	if (max_y < min_y)
		max_y = min_y;
	view->lx = x < min_x ? min_x : (x > max_x ? max_x : x);
	view->ly = y < min_y ? min_y : (y > max_y ? max_y : y);
		      ...
		      double dy_unaccel)
{
	...
	if (!view) {
		/* Rootless XWayland 24.1 may expose a 1x1 Wayland root surface while
		 * keeping its real X window geometry internally. This is a single-app
		 * host, so route the logical pointer through the focused root surface
		 * and let XWayland select its child window. */
		struct cg_view *focused = seat_get_focus(seat);
		if (focused && focused->wlr_surface) {
			view = focused;
			surface = focused->wlr_surface;
			sx = seat->cursor->x - focused->lx;
			sy = seat->cursor->y - focused->ly;
		}
	}
	if (!view) {
...
handle_cursor_motion_absolute(struct wl_listener *listener, void *data)
{
	...
	float coordinate_scale = host_pointer_coordinate_scale(seat, event->pointer);
	double x = event->x * coordinate_scale;
	double y = event->y * coordinate_scale;
	x = x < 0.0 ? 0.0 : (x > 1.0 ? 1.0 : x);
	y = y < 0.0 ? 0.0 : (y > 1.0 ? 1.0 : y);

	double lx, ly;
	wlr_cursor_absolute_to_layout_coords(seat->cursor, &event->pointer->base, x, y, &lx, &ly);
	double dx = lx - seat->cursor->x;
	double dy = ly - seat->cursor->y;

	wlr_cursor_warp_absolute(seat->cursor, &event->pointer->base, x, y);
...
handle_destroy(struct wl_listener *listener, void *data)
{
	...
	if (seat->activity_fd >= 0) {
		close(seat->activity_fd);
	}
	free(seat);
}
...
seat_create(struct cg_server *server, struct wlr_backend *backend)
{
	...
	seat->activity_fd = -1;
	const char *activity_socket = getenv("AGENTSEAT_ACTIVITY_SOCKET");
	if (activity_socket && activity_socket[0] && strlen(activity_socket) < sizeof(seat->activity_socket_path)) {
		seat->activity_fd = socket(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC | SOCK_NONBLOCK, 0);
		if (seat->activity_fd >= 0) {
			strcpy(seat->activity_socket_path, activity_socket);
		} else {
			wlr_log(WLR_ERROR, "Cannot create AgentSeat human activity channel: %s", strerror(errno));
		}
	}
