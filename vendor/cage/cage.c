...
handle_signal(int signal, void *data)
{
	...
	case SIGTERM:
		...
		return 0;
	case SIGUSR1:
		/* AgentSeat capture requests need one render even when the parent
		 * compositor is not presenting this nested single-app window. */
		output_render_all(server);
		return 0;
	default:
		return 0;
	}
...
main(int argc, char *argv[])
{
	...
	/* Clipboard helpers run only on AgentSeat's private Wayland socket and do
	 * not receive a user-input serial. Expose both current and legacy
	 * data-control protocols so they can publish a one-shot private selection
	 * without ever touching the host compositor clipboard. */
	if (!wlr_ext_data_control_manager_v1_create(server.wl_display, 1)) {
		wlr_log(WLR_ERROR, "Unable to create the ext data-control manager");
		ret = 1;
		goto end;
	}
	if (!wlr_data_control_manager_v1_create(server.wl_display)) {
		wlr_log(WLR_ERROR, "Unable to create the legacy data-control manager");
		ret = 1;
		goto end;
	}
	...
	if (!output_init_host_viewporter(&server)) {
		wlr_log(WLR_ERROR, "Unable to initialize host fractional-scale viewport");
		ret = 1;
		goto end;
	}
	...
	output_finish_host_viewporter(&server);
