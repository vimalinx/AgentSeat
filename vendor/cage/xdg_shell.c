...
static bool
is_agentseat_overlay_toplevel(struct wlr_xdg_toplevel *toplevel)
{
	return toplevel->app_id && strcmp(toplevel->app_id, "dev.vimalinx.agentseat.eye") == 0;
}
...
is_primary(struct cg_view *view)
{
	...
	return parent == NULL && !view_is_agentseat_overlay(view);
}
...
{
	struct cg_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, request_move);
	struct wlr_xdg_toplevel_move_event *event = data;
	struct cg_view *view = &xdg_shell_view->view;
	struct cg_seat *seat = view->server->seat;
	if (!view_is_agentseat_overlay(view) ||
	    !wlr_seat_validate_pointer_grab_serial(seat->seat, view->wlr_surface, event->serial))
		return;
	seat->moving_view = view;
	seat->move_offset_x = seat->pointer_press_x - view->lx;
	seat->move_offset_y = seat->pointer_press_y - view->ly;
	wlr_log(WLR_DEBUG,
		"AgentSeat overlay move: press=(%.1f,%.1f) cursor=(%.1f,%.1f) view=(%d,%d) offset=(%.1f,%.1f)",
		seat->pointer_press_x, seat->pointer_press_y, seat->cursor->x, seat->cursor->y,
		view->lx, view->ly, seat->move_offset_x, seat->move_offset_y);
	view->user_positioned = true;
	wlr_scene_node_raise_to_top(&view->scene_tree->node);
}

static void
focus_wrapped_application(struct cg_server *server)
{
	struct cg_view *candidate;
	wl_list_for_each (candidate, &server->views, link) {
		if (!view_is_agentseat_overlay(candidate) && candidate->wlr_surface) {
...
handle_xdg_toplevel_commit(struct wl_listener *listener, void *data)
{
	...
	struct cg_view *view = &xdg_shell_view->view;
	view->agentseat_overlay = is_agentseat_overlay_toplevel(xdg_shell_view->xdg_toplevel);
	...
	wlr_xdg_toplevel_set_wm_capabilities(
		xdg_shell_view->xdg_toplevel,
		view_is_agentseat_overlay(view) ? 0 : XDG_TOPLEVEL_WM_CAPABILITIES_FULLSCREEN);

	if (!view_is_agentseat_overlay(view) && xdg_shell_view->xdg_toplevel->requested.fullscreen) {
...
handle_new_xdg_toplevel(struct wl_listener *listener, void *data)
{
	...
	xdg_shell_view->view.agentseat_overlay = is_agentseat_overlay_toplevel(toplevel);
	...
	xdg_shell_view->request_move.notify = handle_xdg_toplevel_request_move;
